...
public class MainProcessor {
    ...
    public func processPDF(
        ...
        do {
            ...
            // Generate run timestamp for consistent file naming across all components
            let runTimestamp = fileManager.generateRunTimestamp()
            logger.info("Generated run timestamp: \(runTimestamp)")
            
            // Set the same timestamp on the document processor for consistent file naming
            documentProcessor.setRunTimestamp(runTimestamp)
            ...
            // Step 3: Skip generating other output types - only markdown is generated by TraditionalOCRDocumentProcessor
            logger.info("Skipping generation of other output types - only markdown is generated by TraditionalOCRDocumentProcessor")
            
            // Step 6: Update statistics
            let processingTime = Date().timeIntervalSince(startTime)
            
            // Use elements for language detection since we're not generating other outputs
            let languageDetectionText = elements.compactMap { $0.text }.joined(separator: " ")
            let detectedLanguage = languageDetector.detectLanguage(from: languageDetectionText)
            
            // Get the primary output path (markdown generated by TraditionalOCRDocumentProcessor)
            let primaryOutputPath = "markdown generated by TraditionalOCRDocumentProcessor"
            ...
            logger.info("Markdown file generated by TraditionalOCRDocumentProcessor")
    ...
    private func generateAllOutputTypes(from elements: [DocumentElement], inputPath: String, processingResult: DocumentProcessingResult) throws -> [OutputType: String] {
        ...
        // Generate each output type (excluding markdown which is now handled page-by-page)
        for outputType in OutputType.allCases {
            // Skip markdown generation as it's now handled in the page-by-page pipeline
            if outputType == .markdown {
                logger.info("Skipping markdown generation - handled by page-by-page pipeline")
                continue
            }
